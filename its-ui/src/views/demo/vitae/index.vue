<template>
  <div class="app-container">
      ...
      <template v-for='(col, index) in dynamicTableDataColumns'>
        ...
        >
          ...
          <template slot-scope="scope">
            <span v-if="scope.column.property === 'createTime'">
              {{parseTime(scope.row[scope.column.property])}}
            </span>
            <span v-else-if="scope.column.property === 'adeptTechnology'">
              {{adeptTechnologyFormat(scope.row)}}
            </span>
            <span v-else-if="scope.column.property === 'status'">
              {{statusFormat(scope.row)}}
            </span>
            <span v-else-if="scope.column.property === 'sex'">
              {{sexFormat(scope.row)}}
            </span>
            <span v-else>
    ...
    <el-dialog :title="title" :visible.sync="vitaeDetailOpen" width="80%" append-to-body>
      <vitae-detail :vitae-id="checkVitaeId" :dynamicTableDataDetailColumns="dynamicTableDataDetailColumns" :time-stamp="timeStamp"/>
...
import { getCurrentTimestamp } from "@/utils/base.js";
export default {
  name: 'Vitae',
  components: {
    ImageUpload,
