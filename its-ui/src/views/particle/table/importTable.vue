...
<script>
import { listGenTable, importTable } from '@/api/particle/table'
export default {
  props: {
    roleId: {
      type: String,
  ...
  methods: {
    ...
    getList() {
      ...
      listGenTable(this.queryParams).then(res => {
        if (res.code === 200) {
    ...
    /** 导入按钮操作 */
    handleImportTable() {
      importTable({ tables: this.tables.join(','), roleId: this.roleId }).then(res => {
